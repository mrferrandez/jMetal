 ...
 *         
 *         Combined version of PAR and SASS
 * @author Miriam R. Ferrandez 
 *         Juana L. Redondo
 *         
 */
public class PAR_SASS<S extends Solution<?>> extends WASFGA<S> {
	/**
	 *
	 */
...
//	List<Double> referencePoint = null;
	 ...
	 *            Problem to solve
	 */
	public PAR_SASS(Problem<S> problem,
								int populationSize,
								int maxIterations,
	...
	public ASFWASFGA<S> createUtilityFunction() {
		double [][] weights = new double[1][this.getProblem().getNumberOfObjectives()];//{{1.0,1.0}};//,1.0}
		for (int nw=0; nw<this.getProblem().getNumberOfObjectives(); nw++){
			weights[0][nw]=1.0;
		}
...
//	    .print();
	...
	protected List<S> replacement(List<S> population, List<S> offspringPopulation) {
		...
		this.achievementScalarizingFunction.setUtopia(getReferencePoint(jointPopulation));
...
//	    System.out.println("Smin "+ ASFmin);
	    for (int nobj=0; nobj < m; nobj++){
	    	...
	    	auxASFunction.setUtopia(getReferencePoint(jointPopulation));
...
//	    System.out.println(ROIPopulation.size());
		...
		for (int i=0; i < ROIPopulation.size(); i++){
			isnotdominated = localSearch.addToExternalList((S)ROIPopulation.get(i).copy());
        	if (!isnotdominated){
        		dominatedPopulation.add(ROIPopulation.get(i));
    		}
		}
		...
		//	resultPopulation.addAll(dominatedPopulation);
		//}
		S improvedIndividual = improveWithSASS(jointPopulation.get(indexMinImpr));//max
		resultPopulation.add(improvedIndividual);
...
//		.print();
		...
		resultPopulation.addAll(localSearch.getExternalList().getSolutionList());
//		System.out.println("SASSexternalListPopulationSize After Fitting= "+ localSearch.getExternalList().size());	
		
//		if (noROIPopulation.size()!=0){
		while ((resultPopulation.size() < this.getMaxPopulationSize())&&(noROIPopulation.size()!=0)){
	    	...
	    	resultPopulation.add(noROIPopulation.get(indexMin));
	    	noROIPopulation.remove(noROIPopulation.get(indexMin));
	    }
//		}
		while (resultPopulation.size() < this.getMaxPopulationSize()){
			ASFmin = achievementScalarizingFunction.evaluate(dominatedPopulation.get(0),0);
	    	int indexMin = 0;
	    	for (int i =1; i < dominatedPopulation.size(); i++){
	    		double ASFvalue = achievementScalarizingFunction.evaluate(dominatedPopulation.get(i),0);
	    		if(ASFvalue < ASFmin){
	    			ASFmin = ASFvalue;
	    			indexMin = i;
	    		}
	    	}
	    	resultPopulation.add(dominatedPopulation.get(indexMin));
	    	dominatedPopulation.remove(dominatedPopulation.get(indexMin));
	    }

		//Ranking<S> ranking = computeRanking(resultPopulation);
	    if (resultPopulation.size() > this.getMaxPopulationSize()){
	    	CrowdingDistance<S> crowdingDistance = new CrowdingDistance<S>() ;
			crowdingDistance.computeDensityEstimator(resultPopulation);
			Collections.sort(resultPopulation, new CrowdingDistanceComparator<S>()) ;
			List<S> finalPopulation = new ArrayList<>(this.getMaxPopulationSize());
		    int i = 0 ;
		    while (finalPopulation.size() < this.getMaxPopulationSize()) {
		    	finalPopulation.add(resultPopulation.get(i)) ;
		    	i++ ;
		    }
		    resultPopulation = finalPopulation;
	    }

		return resultPopulation;//selectBest(ranking);
	}
...
//		}
	...
	protected void fitExternalList(double[] vectMax){
		Iterator<S> iterator = localSearch.getExternalList().getSolutionList().iterator();
		while (iterator.hasNext()) {
			S listIndividual = iterator.next();
			boolean isIntoROI = true;
			for (int nobj=0; nobj < listIndividual.getNumberOfObjectives(); nobj++){
	    		double aux = listIndividual.getObjective(nobj);
	    		if (aux > vectMax[nobj]){
	    			isIntoROI = false;
	    		}
	    	}
...
//		}
	...
	protected S improveWithSASS(S individual) {
		S bestIndividual = (S) individual.copy();
	...
	@SuppressWarnings("unchecked")
	protected S improveWithSASS(double[] vectMax) {
		S bestIndividual = this.getAllPopulation().get(bestIndex2refPoint);
		
		S improvedIndividual ;
		S maxROIindividual = (S) bestIndividual.copy();
		double rad;
//		double highestDistance = 0.0;
//		int indexRad = 0;
		// Calculate radius
		for (int j = 0; j< bestIndividual.getNumberOfObjectives();j++){
			maxROIindividual.setObjective(j, vectMax[j]);
		}
		rad=distDomain.getNormalizedDistance((DoubleSolution)bestIndividual, (DoubleSolution) maxROIindividual);
		localSearch.setRadius(rad);
		System.out.println("RADIUS: " + rad);
		improvedIndividual = localSearch.execute(bestIndividual);
		this.evaluations += localSearch.getEvaluations();
