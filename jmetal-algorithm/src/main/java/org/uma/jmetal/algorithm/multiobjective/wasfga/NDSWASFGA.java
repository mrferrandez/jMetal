 ...
 *         
 *         Modified version of WASFGA called Non-Dominated Secant WASFGA 
 * 		   Local Search Operator SASS included instead of Mutation and Crossover Operators
 * @author Miriam R. Ferrandez 
 *         Juana L. Redondo
 *         
 */
public class NDSWASFGA<S extends Solution<?>> extends AbstractMOMBI<S> {
	/**
	 *
	 */
	 ...
	 *            Problem to solve
	 */
	public NDSWASFGA(Problem<S> problem,
								int populationSize,
								int maxIterations,
...
//	}
	...
	protected List<S> replacement(List<S> population, List<S> offspringPopulation) {
		...
		List<S> improvedPopulation, secantPopulation;
		...
		if (this.iterations > 1){
			secantPopulation = obtainSecantPopulation();
			jointPopulation.addAll(secantPopulation);
		}
		oldPopulation = new ArrayList<>();
		for (int i=0; i < this.getMaxPopulationSize(); i++){
			oldPopulation.add((S) this.getPopulation().get(i).copy());
		}
...
//		}
	...
	@SuppressWarnings("unchecked")
	protected List<S> obtainSecantPopulation() {
		List<S> secantPopulation = new ArrayList<>();
		double f1, f0, value, nvar, x1, x0, x2;
		nvar = this.getProblem().getNumberOfVariables();
		for (int i=0; i < this.getMaxPopulationSize(); i++){
			DoubleSolution secsol = (DoubleSolution) oldPopulation.get(i).copy();
			f0 = this.achievementScalarizingFunction.evaluate(oldPopulation.get(i), i);
			f1 = this.achievementScalarizingFunction.evaluate(this.getPopulation().get(i), i);
			if (f0 != f1){
				value = -f1/(f1-f0);
				value = value / nvar;
				for (int j=0; j < nvar; j++){
					x0 = (double) oldPopulation.get(i).getVariableValue(j);
					x1 = (double) this.getPopulation().get(i).getVariableValue(j);
					x2 = x1 + value * (x1-x0);
					x2 = Double.max(x2,  ((DoubleSolution) oldPopulation.get(i)).getLowerBound(j));
					x2 = Double.min(x2, ((DoubleSolution) oldPopulation.get(i)).getUpperBound(j));
					secsol.setVariableValue(j, x2);
				}
				secantPopulation.add((S) secsol);
			}
		}
		secantPopulation = evaluatePopulation(secantPopulation);
		this.evaluations += secantPopulation.size();
		new SolutionListOutput(secantPopulation)
	    .setSeparator("\t")
	    .setVarFileOutputContext(new DefaultFileOutputContext("VAR_SEC"+Integer.toString(iterations)+".tsv"))
	    .setFunFileOutputContext(new DefaultFileOutputContext("FUN_SEC"+Integer.toString(iterations)+".tsv"))
	    .print();
		return secantPopulation;
	}
