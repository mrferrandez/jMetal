 ...
 *         
 */
public class WASFGA_ad_OBL<S extends Solution<?>> extends AbstractMOMBI<S> {
	/**
	 *
	 */
	 ...
	 *            Problem to solve
	 */
	public WASFGA_ad_OBL(Problem<S> problem,
								int populationSize,
								int maxIterations,
		...
		while (!isStoppingConditionReached()) {
			if (iterations==1){
				...
				for (int i=0; i < this.getPopulationSize(); i++){
					S opositeIndividual = calculateOBL(this.getPopulation().get(i));
					offspringPopulation.add(opositeIndividual);
				}
...
//		}
	...
	@SuppressWarnings("unchecked")
	protected S calculateOBL(S solution){
		DoubleSolution opositeSolution = (DoubleSolution) solution.copy();
		double x, a, b, xop;
		for (int i=0; i < solution.getNumberOfVariables(); i++){
			x = (double) solution.getVariableValue(i);
			a = ((DoubleSolution)solution).getLowerBound(i);
			b = ((DoubleSolution)solution).getUpperBound(i);
			xop= a + b - x;
			opositeSolution.setVariableValue(i, xop);
		}
		
		return (S) opositeSolution;
	}
