 ...
 */
public class PAR_WASFGA<S extends Solution<?>> extends WASFGA<S> {
	/**
	 *
	 */
...
//	    .print();
	...
	public void run() {
		...
		while (!isStoppingConditionReached()) {
			ParPopulation = calculatePAR(this.getPopulation());
			matingPopulation = selection(ParPopulation);//this.getPopulation()
	...
	protected List<S> calculatePAR(List<S> population){
//		List<S> parPop = new ArrayList<>();
		int m = this.getProblem().getNumberOfObjectives();
		double [][] ones= new double[1][m];
		for (int nw=0; nw<this.getProblem().getNumberOfObjectives(); nw++){
			ones[0][nw]=1.0;
		}
		ASFWASFGA<S> asfRefPoint = new ASFWASFGA<>(ones,referencePoint);
		
		double[] max = new double[m];
		initializeBounds(m);
		updateNadirPoint(population);
		updateReferencePoint(population);
		asfRefPoint.setNadir(getNadirPoint(population));
		asfRefPoint.setUtopia(getReferencePoint(population));
		double ASFmin = asfRefPoint.evaluate(population.get(0),0);
//		int indexMinImpr = 0;
	    for (int i =1; i < population.size(); i++){
	    	double ASFvalue = asfRefPoint.evaluate(population.get(i),0);
	    	if(ASFvalue < ASFmin){
	    		ASFmin = ASFvalue;
//	    		indexMinImpr = i;
	    	}
...
//	    System.out.println("Smin "+ ASFmin);
	    for (int nobj=0; nobj < m; nobj++){
	    	...
	    	ASFWASFGA<S> auxASFunction = new ASFWASFGA<>(ones,refPointAux);
	    	auxASFunction.setNadir(getNadirPoint(population));
	    	auxASFunction.setUtopia(getReferencePoint(population));
	    	double auxASFmin = auxASFunction.evaluate(population.get(0),0);
	        int indexMin = 0;
	        for (int i =1; i < population.size(); i++){
	        	double auxASFvalue = auxASFunction.evaluate(population.get(i),0);
...
//	        System.out.println("ZauxREF "+ refPointAux.get(0)+" "+ refPointAux.get(1));
//	        System.out.println(max[nobj]);
	...
	@Override public String getDescription() {
		return "PAR-Weighting Achievement Scalarizing Function Genetic Algorithm" ;
	}
}
